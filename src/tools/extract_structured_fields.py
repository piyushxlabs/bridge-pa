...
SCHEMA = {
    ...
    "description": "Auto-generated schema for extract_structured_fields"
}

class ExtractStructuredFieldsParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "extract_structured_fields invoked", "extract_structured_fields")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_EXTRACT_STRUCTURED_FIELDS", f"http://localhost:8080/extract_structured_fields")
    try:
