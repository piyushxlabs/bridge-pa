...
SCHEMA = {
    ...
    "description": "Auto-generated schema for apply_interqual_matching"
}

class ApplyInterqualMatchingParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "apply_interqual_matching invoked", "apply_interqual_matching")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_APPLY_INTERQUAL_MATCHING", f"http://localhost:8080/apply_interqual_matching")
    try:
