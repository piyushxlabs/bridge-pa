...
SCHEMA = {
    ...
    "description": "Auto-generated schema for submit_authorization_request"
}

class SubmitAuthorizationRequestParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "submit_authorization_request invoked", "submit_authorization_request")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_SUBMIT_AUTHORIZATION_REQUEST", f"http://localhost:8080/submit_authorization_request")
    try:
