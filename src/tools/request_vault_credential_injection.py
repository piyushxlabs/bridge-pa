...
SCHEMA = {
    ...
    "description": "Auto-generated schema for request_vault_credential_injection"
}

class RequestVaultCredentialInjectionParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "request_vault_credential_injection invoked", "request_vault_credential_injection")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_REQUEST_VAULT_CREDENTIAL_INJECTION", f"http://localhost:8080/request_vault_credential_injection")
    try:
