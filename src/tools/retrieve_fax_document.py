...
SCHEMA = {
    ...
    "description": "Auto-generated schema for retrieve_fax_document"
}

class RetrieveFaxDocumentParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "retrieve_fax_document invoked", "retrieve_fax_document")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_RETRIEVE_FAX_DOCUMENT", f"http://localhost:8080/retrieve_fax_document")
    try:
