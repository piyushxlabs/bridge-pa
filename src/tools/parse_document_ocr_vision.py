...
SCHEMA = {
    ...
    "description": "Auto-generated schema for parse_document_ocr_vision"
}

class ParseDocumentOcrVisionParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "parse_document_ocr_vision invoked", "parse_document_ocr_vision")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_PARSE_DOCUMENT_OCR_VISION", f"http://localhost:8080/parse_document_ocr_vision")
    try:
