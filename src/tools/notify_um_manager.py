...
SCHEMA = {
    "name": "notify_um_manager",
    "description": "Auto-generated schema for notify_um_manager"
}

class NotifyUmManagerParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "notify_um_manager invoked", "notify_um_manager")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_NOTIFY_UM_MANAGER", f"http://localhost:8080/notify_um_manager")
    try:
