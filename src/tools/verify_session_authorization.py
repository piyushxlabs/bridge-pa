...
SCHEMA = {
    ...
    "description": "Auto-generated schema for verify_session_authorization"
}

class VerifySessionAuthorizationParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "verify_session_authorization invoked", "verify_session_authorization")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_VERIFY_SESSION_AUTHORIZATION", f"http://localhost:8080/verify_session_authorization")
    try:
