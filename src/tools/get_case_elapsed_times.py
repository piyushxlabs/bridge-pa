...
SCHEMA = {
    "name": "get_case_elapsed_times",
    "description": "Auto-generated schema for get_case_elapsed_times"
}

class GetCaseElapsedTimesParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "get_case_elapsed_times invoked", "get_case_elapsed_times")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_GET_CASE_ELAPSED_TIMES", f"http://localhost:8080/get_case_elapsed_times")
    try:
