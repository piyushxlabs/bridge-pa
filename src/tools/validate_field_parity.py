...
SCHEMA = {
    ...
    "description": "Auto-generated schema for validate_field_parity"
}

class ValidateFieldParityParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "validate_field_parity invoked", "validate_field_parity")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_VALIDATE_FIELD_PARITY", f"http://localhost:8080/validate_field_parity")
    try:
