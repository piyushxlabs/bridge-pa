...
SCHEMA = {
    ...
    "description": "Auto-generated schema for check_audit_proxy_reachability"
}

class CheckAuditProxyReachabilityParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "check_audit_proxy_reachability invoked", "check_audit_proxy_reachability")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_CHECK_AUDIT_PROXY_REACHABILITY", f"http://localhost:8080/check_audit_proxy_reachability")
    try:
