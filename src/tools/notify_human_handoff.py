...
SCHEMA = {
    ...
    "description": "Auto-generated schema for notify_human_handoff"
}

class NotifyHumanHandoffParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "notify_human_handoff invoked", "notify_human_handoff")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_NOTIFY_HUMAN_HANDOFF", f"http://localhost:8080/notify_human_handoff")
    try:
