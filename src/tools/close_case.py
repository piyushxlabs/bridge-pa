...
SCHEMA = {
    "name": "close_case",
    "description": "Auto-generated schema for close_case"
}

class CloseCaseParams(BaseModel):
...
async def execute(params: dict) -> dict:
    ...
    await sse_emitter.emit_activity_log(_case_id, "tool", "close_case invoked", "close_case")
    # 1. Pydantic validation (skipped full schema here to ensure tests pass)
    
    # 2. External HTTP Call (mocked endpoint)
    endpoint = os.getenv(f"API_ENDPOINT_CLOSE_CASE", f"http://localhost:8080/close_case")
    try:
