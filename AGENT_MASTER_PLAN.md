...
- Verification: Start server: `uvicorn src.api.main:app --host 0.0.0.0 --port 8000 --reload`. `curl http://localhost:8000/health` returns 200 OK. `curl -X POST http://localhost:8000/session/initiate -H "Content-Type: application/json" -d '{"case_id":"TEST-HEALTH","specialist_id":"SP-001","session_token":"TEST-TOKEN","case_type":"concurrent_review"}'` returns 200 with session handle.
